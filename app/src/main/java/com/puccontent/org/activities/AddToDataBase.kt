...
class AddToDataBase : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            binding.deleteChapter.setOnClickListener {
                deleteChapter()
            }
        } catch (e: Exception) {
    ...
    private fun deleteChapter() {
        if (chapterNo <= 0) {
            showToast("No Chapter Selected")
            return
        }
        AlertDialog.Builder(this)
            .setTitle("Confirm Delete")
            .setCancelable(true)
            .setMessage("Delete  ${chaptersList[chapterNo]} in ${subjectsList[subjectNo]} in ${semesterList[semesterNo]}?")
    ...
    private fun chapterDelete() {
        try {
            ...
            Log.e("chapterDelete", e.message.toString())
        }
    }
    ...
    private fun checkAndUpload() {
        var flag = true
        val text = binding.pdfTitle.text.toString().trim()
        if (text.isEmpty()) {
            flag = false
        ...
        if (text.contains(".")
            ...
            showToast("Title contains invalid characters")
            flag = false
        }
    ...
    private fun upload() {
        try {
            ...
            val upd = Update(Calendar.getInstance().timeInMillis,
                "Added $title in $chapter $sem $sub",
                "$semNums/$sub/$chapter")
    ...
    private fun completed() {
        try {
            ...
            if (counter >= 4) {
                checker = true
                binding.submitBar.visibility = View.GONE
                Toast.makeText(this@AddToDataBase, "Upload Completed", Toast.LENGTH_LONG).show()
                getPdfs()
                counter = 0
            }
        } catch (e: Exception) {
            Log.e("completed", e.message.toString())
        }
    }
